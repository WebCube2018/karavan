...
<div class="bx-catalog-element bx-<?=$arParams['TEMPLATE_THEME']?>" id="<?=$itemIds['ID']?>"
	...
	<div class="container-fluid">
		...
		<div class="row">
							...
							foreach ($arParams['PRODUCT_PAY_BLOCK_ORDER'] as $blockName)
							{
									...
									case 'price':
                                        ...
                                        <div class="product-item-detail-info-container">
                                            <? if($arResult['DISPLAY_PROPERTIES']['OLD_PRICE']['VALUE']) {?>
                                                <div class="product-item-detail-price-old"  id="<?=$itemIds['OLD_PRICE_ID']?>" ><span class="old_price_value"><?=$arResult['DISPLAY_PROPERTIES']['OLD_PRICE']['VALUE']?></span> <span>руб.</span></div>
                                                <div class="product-item-detail-price-current" id="<?=$itemIds['PRICE_ID']?>" style="font-size: 30px;"><span class="price_value"><?=$arResult['DISPLAY_PROPERTIES']['PRICE']['VALUE']?></span> <span>руб.</span></div>
                                            <?} else {?>
                                                <div class="product-item-detail-price-current" id="<?=$itemIds['PRICE_ID']?>" style="font-size: 30px;"><span class="price_value"><?=$arResult['DISPLAY_PROPERTIES']['PRICE']['VALUE']?></span> <span>руб.</span></div>
                                            <?}?>
                                        </div>
                                ...
                                ?>
                                    ...
                                    <script type="text/javascript">
                                        var oldIf = <?=$oldPriceCalc?>;

                                        if(oldIf == 1){
                                            window.prices = {
                                                current:<?=$arResult['DISPLAY_PROPERTIES']['PRICE']['VALUE']?>,
                                                old:<?=$arResult['DISPLAY_PROPERTIES']['OLD_PRICE']['VALUE']?>
                                            };
                                        } else {
                                            window.prices = {
                                                current:<?=$arResult['DISPLAY_PROPERTIES']['PRICE']['VALUE']?>
                                        ...
                                        $(function(){
                                            ...
                                            $(".additional_option").on("change",function(event){
                                                calculatePrices();
                                            });
                                    ...
                                    function calculatePrices() {
                                        ...
                                        $(".additional_option:checked").each(function(){
                                            var price = $(this).data('price');
                                            additional_price +=price;
                                        });
                                        var result_price = prices.current + additional_price;
                                        var result_price_old = prices.old + additional_price;

                                        var percent =  $(".payment_option:checked").data('percent');

                                        if (percent>0){
                                            result_price = result_price + ((result_price/100)*percent);
                                            result_price_old = result_price_old + ((result_price_old/100)*percent);
                                        }


                                        result_price = parseInt(result_price);
                                        result_price_old = parseInt(result_price_old);


                                        $('.price_value').text(result_price);
                                        $('.old_price_value').text(result_price_old);
            ...
            <div class="opcii">
                <span class="text-opcii">ДОСТУПНЫЕ ОПЦИИ:</span>
                <div id="input-option111">
                    <? foreach ($arResult['DISPLAY_PROPERTIES']['DOP_OPCII']['LINK_ELEMENT_VALUE'] as $PRODUCT_ID){
                        $db_props = CIBlockElement::GetProperty(
                                $arResult['DISPLAY_PROPERTIES']['DOP_OPCII']['LINK_IBLOCK_ID'],
                            $PRODUCT_ID['ID'],
                            array("sort" => "asc"),
                            Array("CODE"=>"PRICES")
                        );
                        ...
                        <div class="checkbox">
                            <label>
                                <input class="additional_option" type="checkbox" name="option[111][]" value="229" data-points="0" data-prefix="+" data-price="<?=IntVal($ar_props["VALUE"])?>">
                                <img src="<?=$URL = CFile::GetPath($PRODUCT_ID['PREVIEW_PICTURE'])?>" alt="<?=$PRODUCT_ID['NAME']?>" class="img-thumbnail">
                                <?=$PRODUCT_ID['NAME']." ".$ar_props['VALUE']." руб." ?>
                            </label>
                        </div>
                        <? }?>
                    <? } ?>
                ...
                <div class="form-group required">
                    ...
                    <div id="input-option112">
                        <? foreach ($arResult['DISPLAY_PROPERTIES']['SPOSOB_OPLAT']['LINK_ELEMENT_VALUE'] as $PRODUCT_ID){
                            $db_props = CIBlockElement::GetProperty(
                                $arResult['DISPLAY_PROPERTIES']['SPOSOB_OPLAT']['LINK_IBLOCK_ID'],
                                $PRODUCT_ID['ID'],
                                array("sort" => "asc"),
                                Array("CODE"=>"PROCENT")
                            );
                            ...
                            if($ar_props = $db_props->GetNext()){?>
                                <div class="radio">
                                    <label>
                                        <input class="payment_option" type="radio" name="option[112]" value="<?=$PRODUCT_ID['ID']?>" data-prefix="*" data-percent="<?=$ar_props['VALUE']?>">
                                        <?=$PRODUCT_ID['NAME']?>
                                    </label>
                                </div>
                            <? }?>
							...
							<div class="product-item-detail-tab-content" data-entity="tab-container" data-value="properties">
								...
								if (!empty($arResult['DISPLAY_PROPERTIES']))
								{
									...
									<dl class="product-item-detail-properties">
										...
										foreach ($arResult['DISPLAY_PROPERTIES'] as $property)
										{
										    if ($property['CODE'] != 'DOP_OPCII' && $property['CODE'] != 'PRICE' && $property['CODE'] != 'OLD_PRICE' && $property['CODE'] != 'SPOSOB_OPLAT'){
				...
				<tr>
					...
					<td rowspan="2" class="product-item-detail-short-card-price">
						...
						<div class="product-item-detail-price-current" data-entity="panel-price">
                            <span class="price_value"><?=$arResult['DISPLAY_PROPERTIES']['PRICE']['VALUE']?></span> <span>руб.</span>
						</div>
					</td>
					<?
					if ($showAddBtn)
